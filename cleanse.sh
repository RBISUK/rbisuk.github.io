...
set -- *.html
[ -d articles ] && set -- "$@" articles/*.html

for f in "$@"; do
  [ -f "$f" ] || continue
  awk 'BEGIN{IGNORECASE=1} !/disclosure:.*not a law firm/ {print}' "$f" > "$f.tmp" && mv "$f.tmp" "$f"
  grep -qi 'not a law firm' "$f" || awk -v s="$STD" 'BEGIN{d=0} /<body[^>]*>/ && !d {print; print s; d=1; next} {print}' "$f" > "$f.tmp" && mv "$f.tmp" "$f"
done
echo "cleanse done"
